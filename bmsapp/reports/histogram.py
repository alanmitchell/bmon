    ...
    def result(self):
        ...
        if len(values):
            ...
            series_info = ( (None, 'Raw'),
                            (bmsapp.data_util.TsBin(1).bin, '1 Hr Averages'),
                            (bmsapp.data_util.TsBin(2).bin, '2 Hr Avg'),
                            (bmsapp.data_util.TsBin(4).bin, '4 Hr Avg'),
                            (bmsapp.data_util.TsBin(8).bin, '8 Hr Avg'),
                            (bmsapp.data_util.TsBin(24).bin, '1 Day Avg') )
            for avg_func, series_name in series_info:
                if avg_func:
                    avg_series = pd_ser.groupby(avg_func).mean()
                else:
                    avg_series = pd_ser
                one_series = {'data': bmsapp.data_util.histogram_from_series(avg_series), 'name': series_name}
                if series_name != 'Raw':
