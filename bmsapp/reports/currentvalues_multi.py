        ...
        for bldg_info in self.chart_info.chartbuildinginfo_set.filter(building__in=buildings):
            ...
            for b_to_sen in bldg_info.building.bldgtosensor_set.filter(sensor__sensor_id__in=sensors):
                ...
                building_sensor_list.append({ 'group': b_to_sen.sensor_group.title,
                                              ...
                                              'href': '{}?select_group={}&select_bldg={}&select_chart={}&select_sensor={}'.format(self.request.build_absolute_uri('/reports/'), self.request_params['select_group'], bldg_info.building.pk, basechart.TIME_SERIES_CHART_ID, b_to_sen.sensor.id) ,
                                              'building_href': '{}?select_group={}&select_bldg={}'.format(self.request.build_absolute_uri('/reports/'), self.request_params['select_group'], bldg_info.building.pk) ,
