...
AN.chart_makers.TimeSeries = function() {
    ...
    cht.redraw = function() { 
        ...
        var pt_count = 0;      // total points
        var max_ser_count = 0;  // points in longest series
        $.each(cht.server_data.series, function(idx, ser) {
            var ser_ct = ser.data.length;
            if (ser_ct > max_ser_count) max_ser_count = ser_ct;
            pt_count += ser_ct;
        });

        var opt;  // the chart options
        if (pt_count < 15000 && max_ser_count < 5000) {
